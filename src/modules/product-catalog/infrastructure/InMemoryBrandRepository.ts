...
import type { IBrandRepository, IBrandFilterOptions } from "../domain/IBrandRepository.ts";

/**
 * InMemory Repository for Brand
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemoryBrandRepository extends InMemoryRepository<Brand> implements IBrandRepository {
  ...
  public async findByCode(code: string): Promise<Brand | null> {
  ...
  public async findFiltered(options: IBrandFilterOptions = {}): Promise<Brand[]> {
  ...
  public async count(options: IBrandFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: Brand[]): Promise<void> {
