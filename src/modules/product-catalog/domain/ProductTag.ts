...
/**
 * Domain Entity: ProductTag
 * Encapsulates state and business invariants for ProductTag within product-catalog.
 */
export class ProductTag extends Entity<ProductTagProps> {
  private _versionNumber: number = 1;

  public constructor(props: ProductTagProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ProductTagProps, id?: string): ProductTag {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ProductTag name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ProductTag code cannot be empty.");
    }
  ...
  public static reconstitute(props: ProductTagProps, id: string, createdAt: Date, updatedAt: Date): ProductTag {
    return new ProductTag(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ProductTagProps>): void {
