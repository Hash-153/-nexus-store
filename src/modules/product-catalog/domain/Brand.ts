...
/**
 * Domain Entity: Brand
 * Encapsulates state and business invariants for Brand within product-catalog.
 */
export class Brand extends Entity<BrandProps> {
  private _versionNumber: number = 1;

  public constructor(props: BrandProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: BrandProps, id?: string): Brand {
    ...
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("Brand code cannot be empty.");
    }
  ...
  public static reconstitute(props: BrandProps, id: string, createdAt: Date, updatedAt: Date): Brand {
    return new Brand(props, id, createdAt, updatedAt);
  }
