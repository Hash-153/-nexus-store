...
/**
 * Domain Entity: ProductBundle
 * Encapsulates state and business invariants for ProductBundle within product-catalog.
 */
export class ProductBundle extends Entity<ProductBundleProps> {
  private _versionNumber: number = 1;

  public constructor(props: ProductBundleProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ProductBundleProps, id?: string): ProductBundle {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ProductBundle name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ProductBundle code cannot be empty.");
    }
  ...
  public static reconstitute(props: ProductBundleProps, id: string, createdAt: Date, updatedAt: Date): ProductBundle {
    return new ProductBundle(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ProductBundleProps>): void {
