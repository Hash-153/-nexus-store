...
/**
 * Domain Entity: ProductMedia
 * Encapsulates state and business invariants for ProductMedia within product-catalog.
 */
export class ProductMedia extends Entity<ProductMediaProps> {
  private _versionNumber: number = 1;

  public constructor(props: ProductMediaProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ProductMediaProps, id?: string): ProductMedia {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ProductMedia name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ProductMedia code cannot be empty.");
    }
  ...
  public static reconstitute(props: ProductMediaProps, id: string, createdAt: Date, updatedAt: Date): ProductMedia {
    return new ProductMedia(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ProductMediaProps>): void {
