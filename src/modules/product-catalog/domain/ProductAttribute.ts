...
/**
 * Domain Entity: ProductAttribute
 * Encapsulates state and business invariants for ProductAttribute within product-catalog.
 */
export class ProductAttribute extends Entity<ProductAttributeProps> {
  private _versionNumber: number = 1;

  public constructor(props: ProductAttributeProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ProductAttributeProps, id?: string): ProductAttribute {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ProductAttribute name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ProductAttribute code cannot be empty.");
    }
  ...
  public static reconstitute(props: ProductAttributeProps, id: string, createdAt: Date, updatedAt: Date): ProductAttribute {
    return new ProductAttribute(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ProductAttributeProps>): void {
