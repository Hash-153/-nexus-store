...
/**
 * Domain Entity: AttributeValue
 * Encapsulates state and business invariants for AttributeValue within product-catalog.
 */
export class AttributeValue extends Entity<AttributeValueProps> {
  private _versionNumber: number = 1;

  public constructor(props: AttributeValueProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: AttributeValueProps, id?: string): AttributeValue {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("AttributeValue name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("AttributeValue code cannot be empty.");
    }
  ...
  public static reconstitute(props: AttributeValueProps, id: string, createdAt: Date, updatedAt: Date): AttributeValue {
    return new AttributeValue(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<AttributeValueProps>): void {
