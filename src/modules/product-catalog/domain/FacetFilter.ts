...
/**
 * Domain Entity: FacetFilter
 * Encapsulates state and business invariants for FacetFilter within product-catalog.
 */
export class FacetFilter extends Entity<FacetFilterProps> {
  private _versionNumber: number = 1;

  public constructor(props: FacetFilterProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: FacetFilterProps, id?: string): FacetFilter {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("FacetFilter name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("FacetFilter code cannot be empty.");
    }
  ...
  public static reconstitute(props: FacetFilterProps, id: string, createdAt: Date, updatedAt: Date): FacetFilter {
    return new FacetFilter(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<FacetFilterProps>): void {
