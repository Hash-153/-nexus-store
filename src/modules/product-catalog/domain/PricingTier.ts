 ...
 * Encapsulates state and business invariants for PricingTier within product-catalog.
 */
export class PricingTier extends Entity<PricingTierProps> {
  private _versionNumber: number = 1;

  public constructor(props: PricingTierProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PricingTierProps, id?: string): PricingTier {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PricingTier name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PricingTier code cannot be empty.");
    }
  ...
  public static reconstitute(props: PricingTierProps, id: string, createdAt: Date, updatedAt: Date): PricingTier {
    return new PricingTier(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PricingTierProps>): void {
