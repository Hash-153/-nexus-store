 ...
 * Encapsulates state and business invariants for Collection within product-catalog.
 */
export class Collection extends Entity<CollectionProps> {
  private _versionNumber: number = 1;

  public constructor(props: CollectionProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CollectionProps, id?: string): Collection {
    ...
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("Collection code cannot be empty.");
    }
  ...
  public static reconstitute(props: CollectionProps, id: string, createdAt: Date, updatedAt: Date): Collection {
    return new Collection(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CollectionProps>): void {
