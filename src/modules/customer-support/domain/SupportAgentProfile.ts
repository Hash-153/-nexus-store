...
export interface SupportAgentProfileProps {
...
/**
 * Domain Entity: SupportAgentProfile
 * Encapsulates state and business invariants for SupportAgentProfile within customer-support.
 */
export class SupportAgentProfile extends Entity<SupportAgentProfileProps> {
  private _versionNumber: number = 1;

  public constructor(props: SupportAgentProfileProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: SupportAgentProfileProps, id?: string): SupportAgentProfile {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("SupportAgentProfile name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("SupportAgentProfile code cannot be empty.");
    }
  ...
  public static reconstitute(props: SupportAgentProfileProps, id: string, createdAt: Date, updatedAt: Date): SupportAgentProfile {
    return new SupportAgentProfile(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<SupportAgentProfileProps>): void {
