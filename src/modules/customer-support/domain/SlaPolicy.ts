...
/**
 * Domain Entity: SlaPolicy
 * Encapsulates state and business invariants for SlaPolicy within customer-support.
 */
export class SlaPolicy extends Entity<SlaPolicyProps> {
  private _versionNumber: number = 1;

  public constructor(props: SlaPolicyProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: SlaPolicyProps, id?: string): SlaPolicy {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("SlaPolicy name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("SlaPolicy code cannot be empty.");
    }
  ...
  public static reconstitute(props: SlaPolicyProps, id: string, createdAt: Date, updatedAt: Date): SlaPolicy {
    return new SlaPolicy(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<SlaPolicyProps>): void {
