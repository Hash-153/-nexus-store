...
/**
 * Domain Entity: CannedResponse
 * Encapsulates state and business invariants for CannedResponse within customer-support.
 */
export class CannedResponse extends Entity<CannedResponseProps> {
  private _versionNumber: number = 1;

  public constructor(props: CannedResponseProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CannedResponseProps, id?: string): CannedResponse {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CannedResponse name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CannedResponse code cannot be empty.");
    }
  ...
  public static reconstitute(props: CannedResponseProps, id: string, createdAt: Date, updatedAt: Date): CannedResponse {
    return new CannedResponse(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CannedResponseProps>): void {
