...
/**
 * Domain Aggregate Root: SupportTicket
 * Encapsulates state and business invariants for SupportTicket within customer-support.
 */
export class SupportTicket extends AggregateRoot<SupportTicketProps> {
  private _versionNumber: number = 1;

  public constructor(props: SupportTicketProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: SupportTicketProps, id?: string): SupportTicket {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("SupportTicket name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("SupportTicket code cannot be empty.");
    }
  ...
  public static reconstitute(props: SupportTicketProps, id: string, createdAt: Date, updatedAt: Date): SupportTicket {
    return new SupportTicket(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<SupportTicketProps>): void {
