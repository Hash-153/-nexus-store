...
/**
 * Domain Entity: TicketMessage
 * Encapsulates state and business invariants for TicketMessage within customer-support.
 */
export class TicketMessage extends Entity<TicketMessageProps> {
  private _versionNumber: number = 1;

  public constructor(props: TicketMessageProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: TicketMessageProps, id?: string): TicketMessage {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("TicketMessage name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("TicketMessage code cannot be empty.");
    }
  ...
  public static reconstitute(props: TicketMessageProps, id: string, createdAt: Date, updatedAt: Date): TicketMessage {
    return new TicketMessage(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<TicketMessageProps>): void {
