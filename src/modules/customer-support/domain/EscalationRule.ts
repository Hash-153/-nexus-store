...
/**
 * Domain Entity: EscalationRule
 * Encapsulates state and business invariants for EscalationRule within customer-support.
 */
export class EscalationRule extends Entity<EscalationRuleProps> {
  private _versionNumber: number = 1;

  public constructor(props: EscalationRuleProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: EscalationRuleProps, id?: string): EscalationRule {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("EscalationRule name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("EscalationRule code cannot be empty.");
    }
  ...
  public static reconstitute(props: EscalationRuleProps, id: string, createdAt: Date, updatedAt: Date): EscalationRule {
    return new EscalationRule(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<EscalationRuleProps>): void {
