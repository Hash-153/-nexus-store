...
/**
 * Domain Entity: TicketAttachment
 * Encapsulates state and business invariants for TicketAttachment within customer-support.
 */
export class TicketAttachment extends Entity<TicketAttachmentProps> {
  private _versionNumber: number = 1;

  public constructor(props: TicketAttachmentProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: TicketAttachmentProps, id?: string): TicketAttachment {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("TicketAttachment name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("TicketAttachment code cannot be empty.");
    }
  ...
  public static reconstitute(props: TicketAttachmentProps, id: string, createdAt: Date, updatedAt: Date): TicketAttachment {
    return new TicketAttachment(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<TicketAttachmentProps>): void {
