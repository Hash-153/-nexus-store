...
export interface WebhookNotificationProps {
...
/**
 * Domain Entity: WebhookNotification
 * Encapsulates state and business invariants for WebhookNotification within notifications.
 */
export class WebhookNotification extends Entity<WebhookNotificationProps> {
  private _versionNumber: number = 1;

  public constructor(props: WebhookNotificationProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: WebhookNotificationProps, id?: string): WebhookNotification {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("WebhookNotification name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("WebhookNotification code cannot be empty.");
    }
  ...
  public static reconstitute(props: WebhookNotificationProps, id: string, createdAt: Date, updatedAt: Date): WebhookNotification {
    return new WebhookNotification(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<WebhookNotificationProps>): void {
