...
/**
 * Domain Aggregate Root: NotificationTemplate
 * Encapsulates state and business invariants for NotificationTemplate within notifications.
 */
export class NotificationTemplate extends AggregateRoot<NotificationTemplateProps> {
  private _versionNumber: number = 1;

  public constructor(props: NotificationTemplateProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: NotificationTemplateProps, id?: string): NotificationTemplate {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("NotificationTemplate name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("NotificationTemplate code cannot be empty.");
    }
  ...
  public static reconstitute(props: NotificationTemplateProps, id: string, createdAt: Date, updatedAt: Date): NotificationTemplate {
    return new NotificationTemplate(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<NotificationTemplateProps>): void {
