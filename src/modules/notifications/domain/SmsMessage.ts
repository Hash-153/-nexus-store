...
/**
 * Domain Entity: SmsMessage
 * Encapsulates state and business invariants for SmsMessage within notifications.
 */
export class SmsMessage extends Entity<SmsMessageProps> {
  private _versionNumber: number = 1;

  public constructor(props: SmsMessageProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: SmsMessageProps, id?: string): SmsMessage {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("SmsMessage name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("SmsMessage code cannot be empty.");
    }
  ...
  public static reconstitute(props: SmsMessageProps, id: string, createdAt: Date, updatedAt: Date): SmsMessage {
    return new SmsMessage(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<SmsMessageProps>): void {
