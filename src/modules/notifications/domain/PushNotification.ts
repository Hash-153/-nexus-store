...
/**
 * Domain Entity: PushNotification
 * Encapsulates state and business invariants for PushNotification within notifications.
 */
export class PushNotification extends Entity<PushNotificationProps> {
  private _versionNumber: number = 1;

  public constructor(props: PushNotificationProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PushNotificationProps, id?: string): PushNotification {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PushNotification name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PushNotification code cannot be empty.");
    }
  ...
  public static reconstitute(props: PushNotificationProps, id: string, createdAt: Date, updatedAt: Date): PushNotification {
    return new PushNotification(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PushNotificationProps>): void {
