...
/**
 * Domain Entity: EmailMessage
 * Encapsulates state and business invariants for EmailMessage within notifications.
 */
export class EmailMessage extends Entity<EmailMessageProps> {
  private _versionNumber: number = 1;

  public constructor(props: EmailMessageProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: EmailMessageProps, id?: string): EmailMessage {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("EmailMessage name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("EmailMessage code cannot be empty.");
    }
  ...
  public static reconstitute(props: EmailMessageProps, id: string, createdAt: Date, updatedAt: Date): EmailMessage {
    return new EmailMessage(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<EmailMessageProps>): void {
