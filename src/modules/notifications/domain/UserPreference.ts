 ...
 * Encapsulates state and business invariants for UserPreference within notifications.
 */
export class UserPreference extends Entity<UserPreferenceProps> {
  private _versionNumber: number = 1;

  public constructor(props: UserPreferenceProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: UserPreferenceProps, id?: string): UserPreference {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("UserPreference name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("UserPreference code cannot be empty.");
    }
  ...
  public static reconstitute(props: UserPreferenceProps, id: string, createdAt: Date, updatedAt: Date): UserPreference {
    return new UserPreference(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<UserPreferenceProps>): void {
