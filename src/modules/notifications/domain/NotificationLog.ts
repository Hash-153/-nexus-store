 ...
 * Encapsulates state and business invariants for NotificationLog within notifications.
 */
export class NotificationLog extends Entity<NotificationLogProps> {
  private _versionNumber: number = 1;

  public constructor(props: NotificationLogProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: NotificationLogProps, id?: string): NotificationLog {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("NotificationLog name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("NotificationLog code cannot be empty.");
    }
  ...
  public static reconstitute(props: NotificationLogProps, id: string, createdAt: Date, updatedAt: Date): NotificationLog {
    return new NotificationLog(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<NotificationLogProps>): void {
