...
/**
 * Domain Entity: BinLocation
 * Encapsulates state and business invariants for BinLocation within inventory-warehouse.
 */
export class BinLocation extends Entity<BinLocationProps> {
  private _versionNumber: number = 1;

  public constructor(props: BinLocationProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: BinLocationProps, id?: string): BinLocation {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("BinLocation name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("BinLocation code cannot be empty.");
    }
  ...
  public static reconstitute(props: BinLocationProps, id: string, createdAt: Date, updatedAt: Date): BinLocation {
    return new BinLocation(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<BinLocationProps>): void {
