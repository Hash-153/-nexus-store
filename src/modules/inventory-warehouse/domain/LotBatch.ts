...
/**
 * Domain Entity: LotBatch
 * Encapsulates state and business invariants for LotBatch within inventory-warehouse.
 */
export class LotBatch extends Entity<LotBatchProps> {
  private _versionNumber: number = 1;

  public constructor(props: LotBatchProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: LotBatchProps, id?: string): LotBatch {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("LotBatch name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("LotBatch code cannot be empty.");
    }
  ...
  public static reconstitute(props: LotBatchProps, id: string, createdAt: Date, updatedAt: Date): LotBatch {
    return new LotBatch(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<LotBatchProps>): void {
