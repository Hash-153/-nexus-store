...
/**
 * Domain Entity: StockLevel
 * Encapsulates state and business invariants for StockLevel within inventory-warehouse.
 */
export class StockLevel extends Entity<StockLevelProps> {
  private _versionNumber: number = 1;

  public constructor(props: StockLevelProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: StockLevelProps, id?: string): StockLevel {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("StockLevel name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("StockLevel code cannot be empty.");
    }
  ...
  public static reconstitute(props: StockLevelProps, id: string, createdAt: Date, updatedAt: Date): StockLevel {
    return new StockLevel(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<StockLevelProps>): void {
