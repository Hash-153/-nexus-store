 ...
 * Encapsulates state and business invariants for Supplier within inventory-warehouse.
 */
export class Supplier extends Entity<SupplierProps> {
  private _versionNumber: number = 1;

  public constructor(props: SupplierProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: SupplierProps, id?: string): Supplier {
    ...
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("Supplier code cannot be empty.");
    }
  ...
  public static reconstitute(props: SupplierProps, id: string, createdAt: Date, updatedAt: Date): Supplier {
    return new Supplier(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<SupplierProps>): void {
