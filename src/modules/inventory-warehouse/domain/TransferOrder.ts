...
/**
 * Domain Entity: TransferOrder
 * Encapsulates state and business invariants for TransferOrder within inventory-warehouse.
 */
export class TransferOrder extends Entity<TransferOrderProps> {
  private _versionNumber: number = 1;

  public constructor(props: TransferOrderProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: TransferOrderProps, id?: string): TransferOrder {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("TransferOrder name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("TransferOrder code cannot be empty.");
    }
  ...
  public static reconstitute(props: TransferOrderProps, id: string, createdAt: Date, updatedAt: Date): TransferOrder {
    return new TransferOrder(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<TransferOrderProps>): void {
