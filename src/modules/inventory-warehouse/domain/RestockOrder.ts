...
export interface RestockOrderProps {
...
/**
 * Domain Entity: RestockOrder
 * Encapsulates state and business invariants for RestockOrder within inventory-warehouse.
 */
export class RestockOrder extends Entity<RestockOrderProps> {
  private _versionNumber: number = 1;

  public constructor(props: RestockOrderProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: RestockOrderProps, id?: string): RestockOrder {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("RestockOrder name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("RestockOrder code cannot be empty.");
    }
  ...
  public static reconstitute(props: RestockOrderProps, id: string, createdAt: Date, updatedAt: Date): RestockOrder {
    return new RestockOrder(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<RestockOrderProps>): void {
