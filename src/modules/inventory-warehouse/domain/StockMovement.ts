 ...
 * Encapsulates state and business invariants for StockMovement within inventory-warehouse.
 */
export class StockMovement extends Entity<StockMovementProps> {
  private _versionNumber: number = 1;

  public constructor(props: StockMovementProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: StockMovementProps, id?: string): StockMovement {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("StockMovement name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("StockMovement code cannot be empty.");
    }
  ...
  public static reconstitute(props: StockMovementProps, id: string, createdAt: Date, updatedAt: Date): StockMovement {
    return new StockMovement(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<StockMovementProps>): void {
