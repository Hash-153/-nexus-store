...
/**
 * Domain Entity: PurchaseOrder
 * Encapsulates state and business invariants for PurchaseOrder within inventory-warehouse.
 */
export class PurchaseOrder extends Entity<PurchaseOrderProps> {
  private _versionNumber: number = 1;

  public constructor(props: PurchaseOrderProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PurchaseOrderProps, id?: string): PurchaseOrder {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PurchaseOrder name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PurchaseOrder code cannot be empty.");
    }
  ...
  public static reconstitute(props: PurchaseOrderProps, id: string, createdAt: Date, updatedAt: Date): PurchaseOrder {
    return new PurchaseOrder(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PurchaseOrderProps>): void {
