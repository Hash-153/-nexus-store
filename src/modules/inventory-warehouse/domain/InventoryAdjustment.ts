...
/**
 * Domain Entity: InventoryAdjustment
 * Encapsulates state and business invariants for InventoryAdjustment within inventory-warehouse.
 */
export class InventoryAdjustment extends Entity<InventoryAdjustmentProps> {
  private _versionNumber: number = 1;

  public constructor(props: InventoryAdjustmentProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: InventoryAdjustmentProps, id?: string): InventoryAdjustment {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("InventoryAdjustment name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("InventoryAdjustment code cannot be empty.");
    }
  ...
  public static reconstitute(props: InventoryAdjustmentProps, id: string, createdAt: Date, updatedAt: Date): InventoryAdjustment {
    return new InventoryAdjustment(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<InventoryAdjustmentProps>): void {
