...
/**
 * Domain Entity: StockReservation
 * Encapsulates state and business invariants for StockReservation within inventory-warehouse.
 */
export class StockReservation extends Entity<StockReservationProps> {
  private _versionNumber: number = 1;

  public constructor(props: StockReservationProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: StockReservationProps, id?: string): StockReservation {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("StockReservation name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("StockReservation code cannot be empty.");
    }
  ...
  public static reconstitute(props: StockReservationProps, id: string, createdAt: Date, updatedAt: Date): StockReservation {
    return new StockReservation(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<StockReservationProps>): void {
