...
import { StockLevel } from "../domain/StockLevel.ts";
import type { IStockLevelRepository, IStockLevelFilterOptions } from "../domain/IStockLevelRepository.ts";

/**
 * InMemory Repository for StockLevel
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemoryStockLevelRepository extends InMemoryRepository<StockLevel> implements IStockLevelRepository {
  public async findByName(name: string): Promise<StockLevel | null> {
  ...
  public async findByCode(code: string): Promise<StockLevel | null> {
  ...
  public async findFiltered(options: IStockLevelFilterOptions = {}): Promise<StockLevel[]> {
  ...
  public async count(options: IStockLevelFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: StockLevel[]): Promise<void> {
