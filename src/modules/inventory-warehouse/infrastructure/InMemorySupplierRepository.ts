...
import type { ISupplierRepository, ISupplierFilterOptions } from "../domain/ISupplierRepository.ts";

/**
 * InMemory Repository for Supplier
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemorySupplierRepository extends InMemoryRepository<Supplier> implements ISupplierRepository {
  ...
  public async findFiltered(options: ISupplierFilterOptions = {}): Promise<Supplier[]> {
  ...
  public async count(options: ISupplierFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: Supplier[]): Promise<void> {
