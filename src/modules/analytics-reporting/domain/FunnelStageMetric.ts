...
export interface FunnelStageMetricProps {
...
/**
 * Domain Entity: FunnelStageMetric
 * Encapsulates state and business invariants for FunnelStageMetric within analytics-reporting.
 */
export class FunnelStageMetric extends Entity<FunnelStageMetricProps> {
  private _versionNumber: number = 1;

  public constructor(props: FunnelStageMetricProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: FunnelStageMetricProps, id?: string): FunnelStageMetric {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("FunnelStageMetric name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("FunnelStageMetric code cannot be empty.");
    }

    return new FunnelStageMetric(
      {
        ...props,
  ...
  public static reconstitute(props: FunnelStageMetricProps, id: string, createdAt: Date, updatedAt: Date): FunnelStageMetric {
    return new FunnelStageMetric(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<FunnelStageMetricProps>): void {
