...
export interface ProfitMarginMetricProps {
...
/**
 * Domain Entity: ProfitMarginMetric
 * Encapsulates state and business invariants for ProfitMarginMetric within analytics-reporting.
 */
export class ProfitMarginMetric extends Entity<ProfitMarginMetricProps> {
  private _versionNumber: number = 1;

  public constructor(props: ProfitMarginMetricProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ProfitMarginMetricProps, id?: string): ProfitMarginMetric {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ProfitMarginMetric name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ProfitMarginMetric code cannot be empty.");
    }

    return new ProfitMarginMetric(
      {
        ...props,
  ...
  public static reconstitute(props: ProfitMarginMetricProps, id: string, createdAt: Date, updatedAt: Date): ProfitMarginMetric {
    return new ProfitMarginMetric(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ProfitMarginMetricProps>): void {
