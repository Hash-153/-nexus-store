...
export interface ProductPerformanceRecordProps {
...
/**
 * Domain Entity: ProductPerformanceRecord
 * Encapsulates state and business invariants for ProductPerformanceRecord within analytics-reporting.
 */
export class ProductPerformanceRecord extends Entity<ProductPerformanceRecordProps> {
  private _versionNumber: number = 1;

  public constructor(props: ProductPerformanceRecordProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ProductPerformanceRecordProps, id?: string): ProductPerformanceRecord {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ProductPerformanceRecord name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ProductPerformanceRecord code cannot be empty.");
    }

    return new ProductPerformanceRecord(
      {
        ...props,
  ...
  public static reconstitute(props: ProductPerformanceRecordProps, id: string, createdAt: Date, updatedAt: Date): ProductPerformanceRecord {
    return new ProductPerformanceRecord(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ProductPerformanceRecordProps>): void {
