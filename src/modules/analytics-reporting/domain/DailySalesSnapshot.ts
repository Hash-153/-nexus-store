...
export interface DailySalesSnapshotProps {
...
/**
 * Domain Aggregate Root: DailySalesSnapshot
 * Encapsulates state and business invariants for DailySalesSnapshot within analytics-reporting.
 */
export class DailySalesSnapshot extends AggregateRoot<DailySalesSnapshotProps> {
  private _versionNumber: number = 1;

  public constructor(props: DailySalesSnapshotProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: DailySalesSnapshotProps, id?: string): DailySalesSnapshot {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("DailySalesSnapshot name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("DailySalesSnapshot code cannot be empty.");
    }
  ...
  public static reconstitute(props: DailySalesSnapshotProps, id: string, createdAt: Date, updatedAt: Date): DailySalesSnapshot {
    return new DailySalesSnapshot(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<DailySalesSnapshotProps>): void {
