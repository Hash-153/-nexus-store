...
export interface RefundRateMetricProps {
...
/**
 * Domain Entity: RefundRateMetric
 * Encapsulates state and business invariants for RefundRateMetric within analytics-reporting.
 */
export class RefundRateMetric extends Entity<RefundRateMetricProps> {
  private _versionNumber: number = 1;

  public constructor(props: RefundRateMetricProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: RefundRateMetricProps, id?: string): RefundRateMetric {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("RefundRateMetric name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("RefundRateMetric code cannot be empty.");
    }

    return new RefundRateMetric(
      {
        ...props,
  ...
  public static reconstitute(props: RefundRateMetricProps, id: string, createdAt: Date, updatedAt: Date): RefundRateMetric {
    return new RefundRateMetric(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<RefundRateMetricProps>): void {
