...
export interface InventoryTurnoverRecordProps {
...
/**
 * Domain Entity: InventoryTurnoverRecord
 * Encapsulates state and business invariants for InventoryTurnoverRecord within analytics-reporting.
 */
export class InventoryTurnoverRecord extends Entity<InventoryTurnoverRecordProps> {
  private _versionNumber: number = 1;

  public constructor(props: InventoryTurnoverRecordProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: InventoryTurnoverRecordProps, id?: string): InventoryTurnoverRecord {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("InventoryTurnoverRecord name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("InventoryTurnoverRecord code cannot be empty.");
    }

    return new InventoryTurnoverRecord(
      {
        ...props,
  ...
  public static reconstitute(props: InventoryTurnoverRecordProps, id: string, createdAt: Date, updatedAt: Date): InventoryTurnoverRecord {
    return new InventoryTurnoverRecord(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<InventoryTurnoverRecordProps>): void {
