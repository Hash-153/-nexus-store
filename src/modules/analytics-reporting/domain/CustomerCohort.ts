...
export interface CustomerCohortProps {
...
/**
 * Domain Entity: CustomerCohort
 * Encapsulates state and business invariants for CustomerCohort within analytics-reporting.
 */
export class CustomerCohort extends Entity<CustomerCohortProps> {
  private _versionNumber: number = 1;

  public constructor(props: CustomerCohortProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CustomerCohortProps, id?: string): CustomerCohort {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CustomerCohort name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CustomerCohort code cannot be empty.");
    }

    return new CustomerCohort(
      {
        ...props,
  ...
  public static reconstitute(props: CustomerCohortProps, id: string, createdAt: Date, updatedAt: Date): CustomerCohort {
    return new CustomerCohort(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CustomerCohortProps>): void {
