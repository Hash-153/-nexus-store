...
export interface TrafficSourceMetricProps {
...
/**
 * Domain Entity: TrafficSourceMetric
 * Encapsulates state and business invariants for TrafficSourceMetric within analytics-reporting.
 */
export class TrafficSourceMetric extends Entity<TrafficSourceMetricProps> {
  private _versionNumber: number = 1;

  public constructor(props: TrafficSourceMetricProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: TrafficSourceMetricProps, id?: string): TrafficSourceMetric {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("TrafficSourceMetric name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("TrafficSourceMetric code cannot be empty.");
    }

    return new TrafficSourceMetric(
      {
        ...props,
  ...
  public static reconstitute(props: TrafficSourceMetricProps, id: string, createdAt: Date, updatedAt: Date): TrafficSourceMetric {
    return new TrafficSourceMetric(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<TrafficSourceMetricProps>): void {
