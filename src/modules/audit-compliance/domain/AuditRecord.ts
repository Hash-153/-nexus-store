...
/**
 * Domain Aggregate Root: AuditRecord
 * Encapsulates state and business invariants for AuditRecord within audit-compliance.
 */
export class AuditRecord extends AggregateRoot<AuditRecordProps> {
  private _versionNumber: number = 1;

  public constructor(props: AuditRecordProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: AuditRecordProps, id?: string): AuditRecord {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("AuditRecord name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("AuditRecord code cannot be empty.");
    }
  ...
  public static reconstitute(props: AuditRecordProps, id: string, createdAt: Date, updatedAt: Date): AuditRecord {
    return new AuditRecord(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<AuditRecordProps>): void {
