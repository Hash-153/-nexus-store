...
export interface IpAccessLogProps {
...
/**
 * Domain Entity: IpAccessLog
 * Encapsulates state and business invariants for IpAccessLog within audit-compliance.
 */
export class IpAccessLog extends Entity<IpAccessLogProps> {
  private _versionNumber: number = 1;

  public constructor(props: IpAccessLogProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: IpAccessLogProps, id?: string): IpAccessLog {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("IpAccessLog name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("IpAccessLog code cannot be empty.");
    }

    return new IpAccessLog(
      {
        ...props,
  ...
  public static reconstitute(props: IpAccessLogProps, id: string, createdAt: Date, updatedAt: Date): IpAccessLog {
    return new IpAccessLog(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<IpAccessLogProps>): void {
