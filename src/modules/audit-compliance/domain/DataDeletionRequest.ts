...
/**
 * Domain Entity: DataDeletionRequest
 * Encapsulates state and business invariants for DataDeletionRequest within audit-compliance.
 */
export class DataDeletionRequest extends Entity<DataDeletionRequestProps> {
  private _versionNumber: number = 1;

  public constructor(props: DataDeletionRequestProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: DataDeletionRequestProps, id?: string): DataDeletionRequest {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("DataDeletionRequest name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("DataDeletionRequest code cannot be empty.");
    }
  ...
  public static reconstitute(props: DataDeletionRequestProps, id: string, createdAt: Date, updatedAt: Date): DataDeletionRequest {
    return new DataDeletionRequest(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<DataDeletionRequestProps>): void {
