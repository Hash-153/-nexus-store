...
export interface GdprConsentRecordProps {
...
/**
 * Domain Entity: GdprConsentRecord
 * Encapsulates state and business invariants for GdprConsentRecord within audit-compliance.
 */
export class GdprConsentRecord extends Entity<GdprConsentRecordProps> {
  private _versionNumber: number = 1;

  public constructor(props: GdprConsentRecordProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: GdprConsentRecordProps, id?: string): GdprConsentRecord {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("GdprConsentRecord name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("GdprConsentRecord code cannot be empty.");
    }
  ...
  public static reconstitute(props: GdprConsentRecordProps, id: string, createdAt: Date, updatedAt: Date): GdprConsentRecord {
    return new GdprConsentRecord(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<GdprConsentRecordProps>): void {
