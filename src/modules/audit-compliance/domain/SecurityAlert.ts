...
/**
 * Domain Entity: SecurityAlert
 * Encapsulates state and business invariants for SecurityAlert within audit-compliance.
 */
export class SecurityAlert extends Entity<SecurityAlertProps> {
  private _versionNumber: number = 1;

  public constructor(props: SecurityAlertProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: SecurityAlertProps, id?: string): SecurityAlert {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("SecurityAlert name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("SecurityAlert code cannot be empty.");
    }
  ...
  public static reconstitute(props: SecurityAlertProps, id: string, createdAt: Date, updatedAt: Date): SecurityAlert {
    return new SecurityAlert(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<SecurityAlertProps>): void {
