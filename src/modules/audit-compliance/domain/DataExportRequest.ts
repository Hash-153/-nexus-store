...
/**
 * Domain Entity: DataExportRequest
 * Encapsulates state and business invariants for DataExportRequest within audit-compliance.
 */
export class DataExportRequest extends Entity<DataExportRequestProps> {
  private _versionNumber: number = 1;

  public constructor(props: DataExportRequestProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: DataExportRequestProps, id?: string): DataExportRequest {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("DataExportRequest name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("DataExportRequest code cannot be empty.");
    }
  ...
  public static reconstitute(props: DataExportRequestProps, id: string, createdAt: Date, updatedAt: Date): DataExportRequest {
    return new DataExportRequest(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<DataExportRequestProps>): void {
