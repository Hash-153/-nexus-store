...
/**
 * Domain Entity: CompliancePolicy
 * Encapsulates state and business invariants for CompliancePolicy within audit-compliance.
 */
export class CompliancePolicy extends Entity<CompliancePolicyProps> {
  private _versionNumber: number = 1;

  public constructor(props: CompliancePolicyProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CompliancePolicyProps, id?: string): CompliancePolicy {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CompliancePolicy name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CompliancePolicy code cannot be empty.");
    }
  ...
  public static reconstitute(props: CompliancePolicyProps, id: string, createdAt: Date, updatedAt: Date): CompliancePolicy {
    return new CompliancePolicy(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CompliancePolicyProps>): void {
