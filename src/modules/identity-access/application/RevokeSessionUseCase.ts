 ...
 * Coordinates business flow and state transformations for RevokeSession.
 */
export class RevokeSessionUseCase implements IUseCase<RevokeSessionCommand, RevokeSessionResult> {
  constructor(
  ...
  public async execute(command: RevokeSessionCommand): Promise<RevokeSessionResult> {
    if (!command) {
      throw new ValidationError("Command payload is required for RevokeSession.");
    }
    ...
    if (command.id) {
      ...
      return {
        success: true,
        message: "RevokeSession executed successfully on existing Permission.",
    ...
    return {
      success: true,
      message: "RevokeSession created new Permission successfully.",
