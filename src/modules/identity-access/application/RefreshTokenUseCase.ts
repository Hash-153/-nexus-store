 ...
 * Coordinates business flow and state transformations for RefreshToken.
 */
export class RefreshTokenUseCase implements IUseCase<RefreshTokenCommand, RefreshTokenResult> {
  constructor(
  ...
  public async execute(command: RefreshTokenCommand): Promise<RefreshTokenResult> {
    if (!command) {
      throw new ValidationError("Command payload is required for RefreshToken.");
    }
    ...
    if (command.id) {
      ...
      return {
        success: true,
        message: "RefreshToken executed successfully on existing Role.",
    ...
    return {
      success: true,
      message: "RefreshToken created new Role successfully.",
