...
/**
 * Domain Entity: AddressBook
 * Encapsulates state and business invariants for AddressBook within identity-access.
 */
export class AddressBook extends Entity<AddressBookProps> {
  private _versionNumber: number = 1;

  public constructor(props: AddressBookProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: AddressBookProps, id?: string): AddressBook {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("AddressBook name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("AddressBook code cannot be empty.");
    }
  ...
  public static reconstitute(props: AddressBookProps, id: string, createdAt: Date, updatedAt: Date): AddressBook {
    return new AddressBook(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<AddressBookProps>): void {
