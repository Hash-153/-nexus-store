 ...
 * Encapsulates state and business invariants for ApiKey within identity-access.
 */
export class ApiKey extends Entity<ApiKeyProps> {
  private _versionNumber: number = 1;

  public constructor(props: ApiKeyProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ApiKeyProps, id?: string): ApiKey {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ApiKey name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ApiKey code cannot be empty.");
    }
  ...
  public static reconstitute(props: ApiKeyProps, id: string, createdAt: Date, updatedAt: Date): ApiKey {
    return new ApiKey(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ApiKeyProps>): void {
