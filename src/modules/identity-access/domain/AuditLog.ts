 ...
 * Encapsulates state and business invariants for AuditLog within identity-access.
 */
export class AuditLog extends Entity<AuditLogProps> {
  private _versionNumber: number = 1;

  public constructor(props: AuditLogProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: AuditLogProps, id?: string): AuditLog {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("AuditLog name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("AuditLog code cannot be empty.");
    }
  ...
  public static reconstitute(props: AuditLogProps, id: string, createdAt: Date, updatedAt: Date): AuditLog {
    return new AuditLog(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<AuditLogProps>): void {
