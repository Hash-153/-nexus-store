...
export interface MfaConfigProps {
...
/**
 * Domain Entity: MfaConfig
 * Encapsulates state and business invariants for MfaConfig within identity-access.
 */
export class MfaConfig extends Entity<MfaConfigProps> {
  private _versionNumber: number = 1;

  public constructor(props: MfaConfigProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: MfaConfigProps, id?: string): MfaConfig {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("MfaConfig name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("MfaConfig code cannot be empty.");
    }
  ...
  public static reconstitute(props: MfaConfigProps, id: string, createdAt: Date, updatedAt: Date): MfaConfig {
    return new MfaConfig(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<MfaConfigProps>): void {
