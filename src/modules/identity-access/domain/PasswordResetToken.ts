 ...
 * Encapsulates state and business invariants for PasswordResetToken within identity-access.
 */
export class PasswordResetToken extends Entity<PasswordResetTokenProps> {
  private _versionNumber: number = 1;

  public constructor(props: PasswordResetTokenProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PasswordResetTokenProps, id?: string): PasswordResetToken {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PasswordResetToken name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PasswordResetToken code cannot be empty.");
    }
  ...
  public static reconstitute(props: PasswordResetTokenProps, id: string, createdAt: Date, updatedAt: Date): PasswordResetToken {
    return new PasswordResetToken(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PasswordResetTokenProps>): void {
