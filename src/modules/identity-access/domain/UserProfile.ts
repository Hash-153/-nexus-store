 ...
 * Encapsulates state and business invariants for UserProfile within identity-access.
 */
export class UserProfile extends Entity<UserProfileProps> {
  private _versionNumber: number = 1;

  public constructor(props: UserProfileProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: UserProfileProps, id?: string): UserProfile {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("UserProfile name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("UserProfile code cannot be empty.");
    }
  ...
  public static reconstitute(props: UserProfileProps, id: string, createdAt: Date, updatedAt: Date): UserProfile {
    return new UserProfile(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<UserProfileProps>): void {
