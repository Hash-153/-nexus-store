...
import type { IApiKeyRepository, IApiKeyFilterOptions } from "../domain/IApiKeyRepository.ts";

/**
 * InMemory Repository for ApiKey
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemoryApiKeyRepository extends InMemoryRepository<ApiKey> implements IApiKeyRepository {
  public async findByName(name: string): Promise<ApiKey | null> {
  ...
  public async findByCode(code: string): Promise<ApiKey | null> {
  ...
  public async findFiltered(options: IApiKeyFilterOptions = {}): Promise<ApiKey[]> {
  ...
  public async count(options: IApiKeyFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: ApiKey[]): Promise<void> {
