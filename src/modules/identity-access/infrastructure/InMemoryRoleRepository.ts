...
import type { IRoleRepository, IRoleFilterOptions } from "../domain/IRoleRepository.ts";

/**
 * InMemory Repository for Role
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemoryRoleRepository extends InMemoryRepository<Role> implements IRoleRepository {
  ...
  public async findFiltered(options: IRoleFilterOptions = {}): Promise<Role[]> {
  ...
  public async count(options: IRoleFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: Role[]): Promise<void> {
