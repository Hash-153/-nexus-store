...
import type { ISessionRepository, ISessionFilterOptions } from "../domain/ISessionRepository.ts";

/**
 * InMemory Repository for Session
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemorySessionRepository extends InMemoryRepository<Session> implements ISessionRepository {
  public async findByName(name: string): Promise<Session | null> {
  ...
  public async findByCode(code: string): Promise<Session | null> {
  ...
  public async findFiltered(options: ISessionFilterOptions = {}): Promise<Session[]> {
  ...
  public async count(options: ISessionFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: Session[]): Promise<void> {
