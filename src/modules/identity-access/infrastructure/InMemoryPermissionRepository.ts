...
import type { IPermissionRepository, IPermissionFilterOptions } from "../domain/IPermissionRepository.ts";

/**
 * InMemory Repository for Permission
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemoryPermissionRepository extends InMemoryRepository<Permission> implements IPermissionRepository {
  ...
  public async findFiltered(options: IPermissionFilterOptions = {}): Promise<Permission[]> {
  ...
  public async count(options: IPermissionFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: Permission[]): Promise<void> {
