...
import type { IUserProfileRepository, IUserProfileFilterOptions } from "../domain/IUserProfileRepository.ts";

/**
 * InMemory Repository for UserProfile
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemoryUserProfileRepository extends InMemoryRepository<UserProfile> implements IUserProfileRepository {
  ...
  public async findByCode(code: string): Promise<UserProfile | null> {
  ...
  public async findFiltered(options: IUserProfileFilterOptions = {}): Promise<UserProfile[]> {
  ...
  public async count(options: IUserProfileFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: UserProfile[]): Promise<void> {
