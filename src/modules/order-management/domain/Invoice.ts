 ...
 * Encapsulates state and business invariants for Invoice within order-management.
 */
export class Invoice extends Entity<InvoiceProps> {
  private _versionNumber: number = 1;

  public constructor(props: InvoiceProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: InvoiceProps, id?: string): Invoice {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("Invoice name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("Invoice code cannot be empty.");
    }
  ...
  public static reconstitute(props: InvoiceProps, id: string, createdAt: Date, updatedAt: Date): Invoice {
    return new Invoice(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<InvoiceProps>): void {
