...
/**
 * Domain Entity: OrderFulfillment
 * Encapsulates state and business invariants for OrderFulfillment within order-management.
 */
export class OrderFulfillment extends Entity<OrderFulfillmentProps> {
  private _versionNumber: number = 1;

  public constructor(props: OrderFulfillmentProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: OrderFulfillmentProps, id?: string): OrderFulfillment {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("OrderFulfillment name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("OrderFulfillment code cannot be empty.");
    }
  ...
  public static reconstitute(props: OrderFulfillmentProps, id: string, createdAt: Date, updatedAt: Date): OrderFulfillment {
    return new OrderFulfillment(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<OrderFulfillmentProps>): void {
