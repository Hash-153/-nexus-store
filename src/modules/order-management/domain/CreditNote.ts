...
/**
 * Domain Entity: CreditNote
 * Encapsulates state and business invariants for CreditNote within order-management.
 */
export class CreditNote extends Entity<CreditNoteProps> {
  private _versionNumber: number = 1;

  public constructor(props: CreditNoteProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CreditNoteProps, id?: string): CreditNote {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CreditNote name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CreditNote code cannot be empty.");
    }
  ...
  public static reconstitute(props: CreditNoteProps, id: string, createdAt: Date, updatedAt: Date): CreditNote {
    return new CreditNote(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CreditNoteProps>): void {
