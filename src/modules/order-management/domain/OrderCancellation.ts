...
/**
 * Domain Entity: OrderCancellation
 * Encapsulates state and business invariants for OrderCancellation within order-management.
 */
export class OrderCancellation extends Entity<OrderCancellationProps> {
  private _versionNumber: number = 1;

  public constructor(props: OrderCancellationProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: OrderCancellationProps, id?: string): OrderCancellation {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("OrderCancellation name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("OrderCancellation code cannot be empty.");
    }
  ...
  public static reconstitute(props: OrderCancellationProps, id: string, createdAt: Date, updatedAt: Date): OrderCancellation {
    return new OrderCancellation(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<OrderCancellationProps>): void {
