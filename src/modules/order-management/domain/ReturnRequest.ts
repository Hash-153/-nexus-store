...
/**
 * Domain Entity: ReturnRequest
 * Encapsulates state and business invariants for ReturnRequest within order-management.
 */
export class ReturnRequest extends Entity<ReturnRequestProps> {
  private _versionNumber: number = 1;

  public constructor(props: ReturnRequestProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ReturnRequestProps, id?: string): ReturnRequest {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ReturnRequest name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ReturnRequest code cannot be empty.");
    }
  ...
  public static reconstitute(props: ReturnRequestProps, id: string, createdAt: Date, updatedAt: Date): ReturnRequest {
    return new ReturnRequest(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ReturnRequestProps>): void {
