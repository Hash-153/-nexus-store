...
/**
 * Domain Entity: OrderAddress
 * Encapsulates state and business invariants for OrderAddress within order-management.
 */
export class OrderAddress extends Entity<OrderAddressProps> {
  private _versionNumber: number = 1;

  public constructor(props: OrderAddressProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: OrderAddressProps, id?: string): OrderAddress {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("OrderAddress name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("OrderAddress code cannot be empty.");
    }
  ...
  public static reconstitute(props: OrderAddressProps, id: string, createdAt: Date, updatedAt: Date): OrderAddress {
    return new OrderAddress(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<OrderAddressProps>): void {
