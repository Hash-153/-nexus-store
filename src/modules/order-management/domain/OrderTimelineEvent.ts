...
export interface OrderTimelineEventProps {
...
/**
 * Domain Entity: OrderTimelineEvent
 * Encapsulates state and business invariants for OrderTimelineEvent within order-management.
 */
export class OrderTimelineEvent extends Entity<OrderTimelineEventProps> {
  private _versionNumber: number = 1;

  public constructor(props: OrderTimelineEventProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: OrderTimelineEventProps, id?: string): OrderTimelineEvent {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("OrderTimelineEvent name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("OrderTimelineEvent code cannot be empty.");
    }
  ...
  public static reconstitute(props: OrderTimelineEventProps, id: string, createdAt: Date, updatedAt: Date): OrderTimelineEvent {
    return new OrderTimelineEvent(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<OrderTimelineEventProps>): void {
