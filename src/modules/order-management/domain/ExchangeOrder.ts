...
export interface ExchangeOrderProps {
...
/**
 * Domain Entity: ExchangeOrder
 * Encapsulates state and business invariants for ExchangeOrder within order-management.
 */
export class ExchangeOrder extends Entity<ExchangeOrderProps> {
  private _versionNumber: number = 1;

  public constructor(props: ExchangeOrderProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ExchangeOrderProps, id?: string): ExchangeOrder {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ExchangeOrder name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ExchangeOrder code cannot be empty.");
    }
  ...
  public static reconstitute(props: ExchangeOrderProps, id: string, createdAt: Date, updatedAt: Date): ExchangeOrder {
    return new ExchangeOrder(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ExchangeOrderProps>): void {
