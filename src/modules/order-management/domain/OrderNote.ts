...
/**
 * Domain Entity: OrderNote
 * Encapsulates state and business invariants for OrderNote within order-management.
 */
export class OrderNote extends Entity<OrderNoteProps> {
  private _versionNumber: number = 1;

  public constructor(props: OrderNoteProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: OrderNoteProps, id?: string): OrderNote {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("OrderNote name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("OrderNote code cannot be empty.");
    }
  ...
  public static reconstitute(props: OrderNoteProps, id: string, createdAt: Date, updatedAt: Date): OrderNote {
    return new OrderNote(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<OrderNoteProps>): void {
