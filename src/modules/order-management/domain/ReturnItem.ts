...
/**
 * Domain Entity: ReturnItem
 * Encapsulates state and business invariants for ReturnItem within order-management.
 */
export class ReturnItem extends Entity<ReturnItemProps> {
  private _versionNumber: number = 1;

  public constructor(props: ReturnItemProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ReturnItemProps, id?: string): ReturnItem {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ReturnItem name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ReturnItem code cannot be empty.");
    }
  ...
  public static reconstitute(props: ReturnItemProps, id: string, createdAt: Date, updatedAt: Date): ReturnItem {
    return new ReturnItem(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ReturnItemProps>): void {
