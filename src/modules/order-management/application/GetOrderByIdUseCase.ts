 ...
 * Coordinates business flow and state transformations for GetOrderById.
 */
export class GetOrderByIdUseCase implements IUseCase<GetOrderByIdCommand, GetOrderByIdResult> {
  constructor(
    private readonly repository: IOrderItemRepository,
    private readonly eventBus: IEventBus
  ) {}

  public async execute(command: GetOrderByIdCommand): Promise<GetOrderByIdResult> {
    if (!command) {
      throw new ValidationError("Command payload is required for GetOrderById.");
    }
    ...
    if (command.id) {
      ...
      return {
        success: true,
        message: "GetOrderById executed successfully on existing OrderItem.",
    ...
    return {
      success: true,
      message: "GetOrderById created new OrderItem successfully.",
