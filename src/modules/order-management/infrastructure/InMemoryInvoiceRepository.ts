...
import type { IInvoiceRepository, IInvoiceFilterOptions } from "../domain/IInvoiceRepository.ts";

/**
 * InMemory Repository for Invoice
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemoryInvoiceRepository extends InMemoryRepository<Invoice> implements IInvoiceRepository {
  public async findByName(name: string): Promise<Invoice | null> {
  ...
  public async findByCode(code: string): Promise<Invoice | null> {
  ...
  public async findFiltered(options: IInvoiceFilterOptions = {}): Promise<Invoice[]> {
  ...
  public async count(options: IInvoiceFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: Invoice[]): Promise<void> {
