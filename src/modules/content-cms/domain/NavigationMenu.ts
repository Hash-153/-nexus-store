...
/**
 * Domain Entity: NavigationMenu
 * Encapsulates state and business invariants for NavigationMenu within content-cms.
 */
export class NavigationMenu extends Entity<NavigationMenuProps> {
  private _versionNumber: number = 1;

  public constructor(props: NavigationMenuProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: NavigationMenuProps, id?: string): NavigationMenu {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("NavigationMenu name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("NavigationMenu code cannot be empty.");
    }
  ...
  public static reconstitute(props: NavigationMenuProps, id: string, createdAt: Date, updatedAt: Date): NavigationMenu {
    return new NavigationMenu(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<NavigationMenuProps>): void {
