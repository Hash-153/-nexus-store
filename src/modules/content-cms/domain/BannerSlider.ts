...
/**
 * Domain Entity: BannerSlider
 * Encapsulates state and business invariants for BannerSlider within content-cms.
 */
export class BannerSlider extends Entity<BannerSliderProps> {
  private _versionNumber: number = 1;

  public constructor(props: BannerSliderProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: BannerSliderProps, id?: string): BannerSlider {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("BannerSlider name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("BannerSlider code cannot be empty.");
    }
  ...
  public static reconstitute(props: BannerSliderProps, id: string, createdAt: Date, updatedAt: Date): BannerSlider {
    return new BannerSlider(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<BannerSliderProps>): void {
