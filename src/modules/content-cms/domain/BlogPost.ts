 ...
 * Encapsulates state and business invariants for BlogPost within content-cms.
 */
export class BlogPost extends Entity<BlogPostProps> {
  private _versionNumber: number = 1;

  public constructor(props: BlogPostProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: BlogPostProps, id?: string): BlogPost {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("BlogPost name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("BlogPost code cannot be empty.");
    }
  ...
  public static reconstitute(props: BlogPostProps, id: string, createdAt: Date, updatedAt: Date): BlogPost {
    return new BlogPost(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<BlogPostProps>): void {
