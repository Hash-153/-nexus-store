...
/**
 * Domain Entity: FaqItem
 * Encapsulates state and business invariants for FaqItem within content-cms.
 */
export class FaqItem extends Entity<FaqItemProps> {
  private _versionNumber: number = 1;

  public constructor(props: FaqItemProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: FaqItemProps, id?: string): FaqItem {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("FaqItem name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("FaqItem code cannot be empty.");
    }
  ...
  public static reconstitute(props: FaqItemProps, id: string, createdAt: Date, updatedAt: Date): FaqItem {
    return new FaqItem(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<FaqItemProps>): void {
