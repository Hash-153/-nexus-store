...
/**
 * Domain Entity: MediaAsset
 * Encapsulates state and business invariants for MediaAsset within content-cms.
 */
export class MediaAsset extends Entity<MediaAssetProps> {
  private _versionNumber: number = 1;

  public constructor(props: MediaAssetProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: MediaAssetProps, id?: string): MediaAsset {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("MediaAsset name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("MediaAsset code cannot be empty.");
    }
  ...
  public static reconstitute(props: MediaAssetProps, id: string, createdAt: Date, updatedAt: Date): MediaAsset {
    return new MediaAsset(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<MediaAssetProps>): void {
