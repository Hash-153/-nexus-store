 ...
 * Encapsulates state and business invariants for MenuItem within content-cms.
 */
export class MenuItem extends Entity<MenuItemProps> {
  private _versionNumber: number = 1;

  public constructor(props: MenuItemProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: MenuItemProps, id?: string): MenuItem {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("MenuItem name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("MenuItem code cannot be empty.");
    }
  ...
  public static reconstitute(props: MenuItemProps, id: string, createdAt: Date, updatedAt: Date): MenuItem {
    return new MenuItem(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<MenuItemProps>): void {
