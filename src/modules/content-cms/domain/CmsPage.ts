...
/**
 * Domain Aggregate Root: CmsPage
 * Encapsulates state and business invariants for CmsPage within content-cms.
 */
export class CmsPage extends AggregateRoot<CmsPageProps> {
  private _versionNumber: number = 1;

  public constructor(props: CmsPageProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CmsPageProps, id?: string): CmsPage {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CmsPage name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CmsPage code cannot be empty.");
    }
  ...
  public static reconstitute(props: CmsPageProps, id: string, createdAt: Date, updatedAt: Date): CmsPage {
    return new CmsPage(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CmsPageProps>): void {
