...
/**
 * Domain Entity: SeoMetadata
 * Encapsulates state and business invariants for SeoMetadata within content-cms.
 */
export class SeoMetadata extends Entity<SeoMetadataProps> {
  private _versionNumber: number = 1;

  public constructor(props: SeoMetadataProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: SeoMetadataProps, id?: string): SeoMetadata {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("SeoMetadata name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("SeoMetadata code cannot be empty.");
    }
  ...
  public static reconstitute(props: SeoMetadataProps, id: string, createdAt: Date, updatedAt: Date): SeoMetadata {
    return new SeoMetadata(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<SeoMetadataProps>): void {
