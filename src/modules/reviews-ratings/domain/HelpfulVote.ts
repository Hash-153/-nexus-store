...
/**
 * Domain Entity: HelpfulVote
 * Encapsulates state and business invariants for HelpfulVote within reviews-ratings.
 */
export class HelpfulVote extends Entity<HelpfulVoteProps> {
  private _versionNumber: number = 1;

  public constructor(props: HelpfulVoteProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: HelpfulVoteProps, id?: string): HelpfulVote {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("HelpfulVote name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("HelpfulVote code cannot be empty.");
    }
  ...
  public static reconstitute(props: HelpfulVoteProps, id: string, createdAt: Date, updatedAt: Date): HelpfulVote {
    return new HelpfulVote(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<HelpfulVoteProps>): void {
