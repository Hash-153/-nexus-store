...
/**
 * Domain Entity: RatingSummary
 * Encapsulates state and business invariants for RatingSummary within reviews-ratings.
 */
export class RatingSummary extends Entity<RatingSummaryProps> {
  private _versionNumber: number = 1;

  public constructor(props: RatingSummaryProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: RatingSummaryProps, id?: string): RatingSummary {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("RatingSummary name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("RatingSummary code cannot be empty.");
    }
  ...
  public static reconstitute(props: RatingSummaryProps, id: string, createdAt: Date, updatedAt: Date): RatingSummary {
    return new RatingSummary(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<RatingSummaryProps>): void {
