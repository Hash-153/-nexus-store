...
/**
 * Domain Entity: CustomerBadge
 * Encapsulates state and business invariants for CustomerBadge within reviews-ratings.
 */
export class CustomerBadge extends Entity<CustomerBadgeProps> {
  private _versionNumber: number = 1;

  public constructor(props: CustomerBadgeProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CustomerBadgeProps, id?: string): CustomerBadge {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CustomerBadge name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CustomerBadge code cannot be empty.");
    }

    return new CustomerBadge(
      {
        ...props,
  ...
  public static reconstitute(props: CustomerBadgeProps, id: string, createdAt: Date, updatedAt: Date): CustomerBadge {
    return new CustomerBadge(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CustomerBadgeProps>): void {
