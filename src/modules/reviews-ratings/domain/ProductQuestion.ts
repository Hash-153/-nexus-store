...
/**
 * Domain Entity: ProductQuestion
 * Encapsulates state and business invariants for ProductQuestion within reviews-ratings.
 */
export class ProductQuestion extends Entity<ProductQuestionProps> {
  private _versionNumber: number = 1;

  public constructor(props: ProductQuestionProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ProductQuestionProps, id?: string): ProductQuestion {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ProductQuestion name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ProductQuestion code cannot be empty.");
    }
  ...
  public static reconstitute(props: ProductQuestionProps, id: string, createdAt: Date, updatedAt: Date): ProductQuestion {
    return new ProductQuestion(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ProductQuestionProps>): void {
