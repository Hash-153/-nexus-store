...
/**
 * Domain Entity: ReviewMedia
 * Encapsulates state and business invariants for ReviewMedia within reviews-ratings.
 */
export class ReviewMedia extends Entity<ReviewMediaProps> {
  private _versionNumber: number = 1;

  public constructor(props: ReviewMediaProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ReviewMediaProps, id?: string): ReviewMedia {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ReviewMedia name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ReviewMedia code cannot be empty.");
    }
  ...
  public static reconstitute(props: ReviewMediaProps, id: string, createdAt: Date, updatedAt: Date): ReviewMedia {
    return new ReviewMedia(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ReviewMediaProps>): void {
