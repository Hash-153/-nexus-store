...
/**
 * Domain Entity: ReviewModerationItem
 * Encapsulates state and business invariants for ReviewModerationItem within reviews-ratings.
 */
export class ReviewModerationItem extends Entity<ReviewModerationItemProps> {
  private _versionNumber: number = 1;

  public constructor(props: ReviewModerationItemProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ReviewModerationItemProps, id?: string): ReviewModerationItem {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ReviewModerationItem name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ReviewModerationItem code cannot be empty.");
    }

    return new ReviewModerationItem(
      {
        ...props,
  ...
  public static reconstitute(props: ReviewModerationItemProps, id: string, createdAt: Date, updatedAt: Date): ReviewModerationItem {
    return new ReviewModerationItem(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ReviewModerationItemProps>): void {
