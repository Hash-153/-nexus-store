...
export interface DeliverySlotProps {
...
/**
 * Domain Entity: DeliverySlot
 * Encapsulates state and business invariants for DeliverySlot within shipping-logistics.
 */
export class DeliverySlot extends Entity<DeliverySlotProps> {
  private _versionNumber: number = 1;

  public constructor(props: DeliverySlotProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: DeliverySlotProps, id?: string): DeliverySlot {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("DeliverySlot name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("DeliverySlot code cannot be empty.");
    }
  ...
  public static reconstitute(props: DeliverySlotProps, id: string, createdAt: Date, updatedAt: Date): DeliverySlot {
    return new DeliverySlot(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<DeliverySlotProps>): void {
