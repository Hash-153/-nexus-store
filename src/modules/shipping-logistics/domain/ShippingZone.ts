...
/**
 * Domain Entity: ShippingZone
 * Encapsulates state and business invariants for ShippingZone within shipping-logistics.
 */
export class ShippingZone extends Entity<ShippingZoneProps> {
  private _versionNumber: number = 1;

  public constructor(props: ShippingZoneProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ShippingZoneProps, id?: string): ShippingZone {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ShippingZone name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ShippingZone code cannot be empty.");
    }
  ...
  public static reconstitute(props: ShippingZoneProps, id: string, createdAt: Date, updatedAt: Date): ShippingZone {
    return new ShippingZone(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ShippingZoneProps>): void {
