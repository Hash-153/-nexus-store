...
/**
 * Domain Entity: Carrier
 * Encapsulates state and business invariants for Carrier within shipping-logistics.
 */
export class Carrier extends Entity<CarrierProps> {
  private _versionNumber: number = 1;

  public constructor(props: CarrierProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CarrierProps, id?: string): Carrier {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("Carrier name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("Carrier code cannot be empty.");
    }
  ...
  public static reconstitute(props: CarrierProps, id: string, createdAt: Date, updatedAt: Date): Carrier {
    return new Carrier(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CarrierProps>): void {
