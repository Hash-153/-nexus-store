...
/**
 * Domain Entity: CustomsDeclaration
 * Encapsulates state and business invariants for CustomsDeclaration within shipping-logistics.
 */
export class CustomsDeclaration extends Entity<CustomsDeclarationProps> {
  private _versionNumber: number = 1;

  public constructor(props: CustomsDeclarationProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CustomsDeclarationProps, id?: string): CustomsDeclaration {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CustomsDeclaration name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CustomsDeclaration code cannot be empty.");
    }
  ...
  public static reconstitute(props: CustomsDeclarationProps, id: string, createdAt: Date, updatedAt: Date): CustomsDeclaration {
    return new CustomsDeclaration(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CustomsDeclarationProps>): void {
