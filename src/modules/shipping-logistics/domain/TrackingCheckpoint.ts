...
export interface TrackingCheckpointProps {
...
/**
 * Domain Entity: TrackingCheckpoint
 * Encapsulates state and business invariants for TrackingCheckpoint within shipping-logistics.
 */
export class TrackingCheckpoint extends Entity<TrackingCheckpointProps> {
  private _versionNumber: number = 1;

  public constructor(props: TrackingCheckpointProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: TrackingCheckpointProps, id?: string): TrackingCheckpoint {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("TrackingCheckpoint name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("TrackingCheckpoint code cannot be empty.");
    }
  ...
  public static reconstitute(props: TrackingCheckpointProps, id: string, createdAt: Date, updatedAt: Date): TrackingCheckpoint {
    return new TrackingCheckpoint(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<TrackingCheckpointProps>): void {
