...
/**
 * Domain Entity: RateTable
 * Encapsulates state and business invariants for RateTable within shipping-logistics.
 */
export class RateTable extends Entity<RateTableProps> {
  private _versionNumber: number = 1;

  public constructor(props: RateTableProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: RateTableProps, id?: string): RateTable {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("RateTable name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("RateTable code cannot be empty.");
    }
  ...
  public static reconstitute(props: RateTableProps, id: string, createdAt: Date, updatedAt: Date): RateTable {
    return new RateTable(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<RateTableProps>): void {
