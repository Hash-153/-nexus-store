...
/**
 * Domain Entity: PickupLocation
 * Encapsulates state and business invariants for PickupLocation within shipping-logistics.
 */
export class PickupLocation extends Entity<PickupLocationProps> {
  private _versionNumber: number = 1;

  public constructor(props: PickupLocationProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PickupLocationProps, id?: string): PickupLocation {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PickupLocation name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PickupLocation code cannot be empty.");
    }
  ...
  public static reconstitute(props: PickupLocationProps, id: string, createdAt: Date, updatedAt: Date): PickupLocation {
    return new PickupLocation(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PickupLocationProps>): void {
