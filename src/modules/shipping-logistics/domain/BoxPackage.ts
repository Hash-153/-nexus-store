...
/**
 * Domain Entity: BoxPackage
 * Encapsulates state and business invariants for BoxPackage within shipping-logistics.
 */
export class BoxPackage extends Entity<BoxPackageProps> {
  private _versionNumber: number = 1;

  public constructor(props: BoxPackageProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: BoxPackageProps, id?: string): BoxPackage {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("BoxPackage name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("BoxPackage code cannot be empty.");
    }
  ...
  public static reconstitute(props: BoxPackageProps, id: string, createdAt: Date, updatedAt: Date): BoxPackage {
    return new BoxPackage(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<BoxPackageProps>): void {
