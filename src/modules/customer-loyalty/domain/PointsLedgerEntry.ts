...
/**
 * Domain Entity: PointsLedgerEntry
 * Encapsulates state and business invariants for PointsLedgerEntry within customer-loyalty.
 */
export class PointsLedgerEntry extends Entity<PointsLedgerEntryProps> {
  private _versionNumber: number = 1;

  public constructor(props: PointsLedgerEntryProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PointsLedgerEntryProps, id?: string): PointsLedgerEntry {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PointsLedgerEntry name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PointsLedgerEntry code cannot be empty.");
    }
  ...
  public static reconstitute(props: PointsLedgerEntryProps, id: string, createdAt: Date, updatedAt: Date): PointsLedgerEntry {
    return new PointsLedgerEntry(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PointsLedgerEntryProps>): void {
