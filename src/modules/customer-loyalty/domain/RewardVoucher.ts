...
export interface RewardVoucherProps {
...
/**
 * Domain Entity: RewardVoucher
 * Encapsulates state and business invariants for RewardVoucher within customer-loyalty.
 */
export class RewardVoucher extends Entity<RewardVoucherProps> {
  private _versionNumber: number = 1;

  public constructor(props: RewardVoucherProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: RewardVoucherProps, id?: string): RewardVoucher {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("RewardVoucher name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("RewardVoucher code cannot be empty.");
    }

    return new RewardVoucher(
      {
        ...props,
  ...
  public static reconstitute(props: RewardVoucherProps, id: string, createdAt: Date, updatedAt: Date): RewardVoucher {
    return new RewardVoucher(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<RewardVoucherProps>): void {
