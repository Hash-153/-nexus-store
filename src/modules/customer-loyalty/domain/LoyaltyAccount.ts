...
/**
 * Domain Aggregate Root: LoyaltyAccount
 * Encapsulates state and business invariants for LoyaltyAccount within customer-loyalty.
 */
export class LoyaltyAccount extends AggregateRoot<LoyaltyAccountProps> {
  private _versionNumber: number = 1;

  public constructor(props: LoyaltyAccountProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: LoyaltyAccountProps, id?: string): LoyaltyAccount {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("LoyaltyAccount name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("LoyaltyAccount code cannot be empty.");
    }
  ...
  public static reconstitute(props: LoyaltyAccountProps, id: string, createdAt: Date, updatedAt: Date): LoyaltyAccount {
    return new LoyaltyAccount(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<LoyaltyAccountProps>): void {
