...
export interface PointsExpirationPolicyProps {
...
/**
 * Domain Entity: PointsExpirationPolicy
 * Encapsulates state and business invariants for PointsExpirationPolicy within customer-loyalty.
 */
export class PointsExpirationPolicy extends Entity<PointsExpirationPolicyProps> {
  private _versionNumber: number = 1;

  public constructor(props: PointsExpirationPolicyProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PointsExpirationPolicyProps, id?: string): PointsExpirationPolicy {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PointsExpirationPolicy name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PointsExpirationPolicy code cannot be empty.");
    }

    return new PointsExpirationPolicy(
      {
        ...props,
  ...
  public static reconstitute(props: PointsExpirationPolicyProps, id: string, createdAt: Date, updatedAt: Date): PointsExpirationPolicy {
    return new PointsExpirationPolicy(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PointsExpirationPolicyProps>): void {
