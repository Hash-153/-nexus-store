...
export interface ReferralRecordProps {
...
/**
 * Domain Entity: ReferralRecord
 * Encapsulates state and business invariants for ReferralRecord within customer-loyalty.
 */
export class ReferralRecord extends Entity<ReferralRecordProps> {
  private _versionNumber: number = 1;

  public constructor(props: ReferralRecordProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ReferralRecordProps, id?: string): ReferralRecord {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ReferralRecord name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ReferralRecord code cannot be empty.");
    }
  ...
  public static reconstitute(props: ReferralRecordProps, id: string, createdAt: Date, updatedAt: Date): ReferralRecord {
    return new ReferralRecord(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ReferralRecordProps>): void {
