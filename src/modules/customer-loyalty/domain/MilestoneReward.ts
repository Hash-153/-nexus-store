...
/**
 * Domain Entity: MilestoneReward
 * Encapsulates state and business invariants for MilestoneReward within customer-loyalty.
 */
export class MilestoneReward extends Entity<MilestoneRewardProps> {
  private _versionNumber: number = 1;

  public constructor(props: MilestoneRewardProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: MilestoneRewardProps, id?: string): MilestoneReward {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("MilestoneReward name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("MilestoneReward code cannot be empty.");
    }
  ...
  public static reconstitute(props: MilestoneRewardProps, id: string, createdAt: Date, updatedAt: Date): MilestoneReward {
    return new MilestoneReward(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<MilestoneRewardProps>): void {
