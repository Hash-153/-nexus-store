...
/**
 * Domain Entity: LoyaltyTier
 * Encapsulates state and business invariants for LoyaltyTier within customer-loyalty.
 */
export class LoyaltyTier extends Entity<LoyaltyTierProps> {
  private _versionNumber: number = 1;

  public constructor(props: LoyaltyTierProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: LoyaltyTierProps, id?: string): LoyaltyTier {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("LoyaltyTier name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("LoyaltyTier code cannot be empty.");
    }
  ...
  public static reconstitute(props: LoyaltyTierProps, id: string, createdAt: Date, updatedAt: Date): LoyaltyTier {
    return new LoyaltyTier(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<LoyaltyTierProps>): void {
