...
/**
 * Domain Entity: CurrencyConversion
 * Encapsulates state and business invariants for CurrencyConversion within cart-checkout.
 */
export class CurrencyConversion extends Entity<CurrencyConversionProps> {
  private _versionNumber: number = 1;

  public constructor(props: CurrencyConversionProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CurrencyConversionProps, id?: string): CurrencyConversion {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CurrencyConversion name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CurrencyConversion code cannot be empty.");
    }
  ...
  public static reconstitute(props: CurrencyConversionProps, id: string, createdAt: Date, updatedAt: Date): CurrencyConversion {
    return new CurrencyConversion(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CurrencyConversionProps>): void {
