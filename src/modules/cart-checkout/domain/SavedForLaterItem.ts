...
/**
 * Domain Entity: SavedForLaterItem
 * Encapsulates state and business invariants for SavedForLaterItem within cart-checkout.
 */
export class SavedForLaterItem extends Entity<SavedForLaterItemProps> {
  private _versionNumber: number = 1;

  public constructor(props: SavedForLaterItemProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: SavedForLaterItemProps, id?: string): SavedForLaterItem {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("SavedForLaterItem name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("SavedForLaterItem code cannot be empty.");
    }

    return new SavedForLaterItem(
      {
        ...props,
  ...
  public static reconstitute(props: SavedForLaterItemProps, id: string, createdAt: Date, updatedAt: Date): SavedForLaterItem {
    return new SavedForLaterItem(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<SavedForLaterItemProps>): void {
