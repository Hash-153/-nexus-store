...
/**
 * Domain Entity: ShippingOption
 * Encapsulates state and business invariants for ShippingOption within cart-checkout.
 */
export class ShippingOption extends Entity<ShippingOptionProps> {
  private _versionNumber: number = 1;

  public constructor(props: ShippingOptionProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ShippingOptionProps, id?: string): ShippingOption {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ShippingOption name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ShippingOption code cannot be empty.");
    }
  ...
  public static reconstitute(props: ShippingOptionProps, id: string, createdAt: Date, updatedAt: Date): ShippingOption {
    return new ShippingOption(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ShippingOptionProps>): void {
