...
/**
 * Domain Entity: CheckoutSession
 * Encapsulates state and business invariants for CheckoutSession within cart-checkout.
 */
export class CheckoutSession extends Entity<CheckoutSessionProps> {
  private _versionNumber: number = 1;

  public constructor(props: CheckoutSessionProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CheckoutSessionProps, id?: string): CheckoutSession {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CheckoutSession name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CheckoutSession code cannot be empty.");
    }
  ...
  public static reconstitute(props: CheckoutSessionProps, id: string, createdAt: Date, updatedAt: Date): CheckoutSession {
    return new CheckoutSession(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CheckoutSessionProps>): void {
