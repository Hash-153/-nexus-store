...
/**
 * Domain Entity: TaxBreakdown
 * Encapsulates state and business invariants for TaxBreakdown within cart-checkout.
 */
export class TaxBreakdown extends Entity<TaxBreakdownProps> {
  private _versionNumber: number = 1;

  public constructor(props: TaxBreakdownProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: TaxBreakdownProps, id?: string): TaxBreakdown {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("TaxBreakdown name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("TaxBreakdown code cannot be empty.");
    }
  ...
  public static reconstitute(props: TaxBreakdownProps, id: string, createdAt: Date, updatedAt: Date): TaxBreakdown {
    return new TaxBreakdown(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<TaxBreakdownProps>): void {
