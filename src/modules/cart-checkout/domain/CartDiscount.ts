...
/**
 * Domain Entity: CartDiscount
 * Encapsulates state and business invariants for CartDiscount within cart-checkout.
 */
export class CartDiscount extends Entity<CartDiscountProps> {
  private _versionNumber: number = 1;

  public constructor(props: CartDiscountProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CartDiscountProps, id?: string): CartDiscount {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CartDiscount name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CartDiscount code cannot be empty.");
    }
  ...
  public static reconstitute(props: CartDiscountProps, id: string, createdAt: Date, updatedAt: Date): CartDiscount {
    return new CartDiscount(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CartDiscountProps>): void {
