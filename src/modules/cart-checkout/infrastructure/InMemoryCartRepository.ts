...
import type { ICartRepository, ICartFilterOptions } from "../domain/ICartRepository.ts";

/**
 * InMemory Repository for Cart
...
export class InMemoryCartRepository extends InMemoryRepository<Cart> implements ICartRepository {
  public async findByName(name: string): Promise<Cart | null> {
  ...
  public async findByCode(code: string): Promise<Cart | null> {
  ...
  public async findFiltered(options: ICartFilterOptions = {}): Promise<Cart[]> {
  ...
  public async count(options: ICartFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: Cart[]): Promise<void> {
