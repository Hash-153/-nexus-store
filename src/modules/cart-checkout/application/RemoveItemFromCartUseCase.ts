 ...
 * Coordinates business flow and state transformations for RemoveItemFromCart.
 */
export class RemoveItemFromCartUseCase implements IUseCase<RemoveItemFromCartCommand, RemoveItemFromCartResult> {
  constructor(
    private readonly repository: ICheckoutSessionRepository,
    private readonly eventBus: IEventBus
  ) {}

  public async execute(command: RemoveItemFromCartCommand): Promise<RemoveItemFromCartResult> {
    if (!command) {
      throw new ValidationError("Command payload is required for RemoveItemFromCart.");
    }
    ...
    if (command.id) {
      ...
      return {
        success: true,
        message: "RemoveItemFromCart executed successfully on existing CheckoutSession.",
    ...
    return {
      success: true,
      message: "RemoveItemFromCart created new CheckoutSession successfully.",
