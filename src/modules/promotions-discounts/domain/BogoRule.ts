...
export interface BogoRuleProps {
...
/**
 * Domain Entity: BogoRule
 * Encapsulates state and business invariants for BogoRule within promotions-discounts.
 */
export class BogoRule extends Entity<BogoRuleProps> {
  private _versionNumber: number = 1;

  public constructor(props: BogoRuleProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: BogoRuleProps, id?: string): BogoRule {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("BogoRule name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("BogoRule code cannot be empty.");
    }
  ...
  public static reconstitute(props: BogoRuleProps, id: string, createdAt: Date, updatedAt: Date): BogoRule {
    return new BogoRule(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<BogoRuleProps>): void {
