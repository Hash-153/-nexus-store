...
/**
 * Domain Entity: CouponUsageRecord
 * Encapsulates state and business invariants for CouponUsageRecord within promotions-discounts.
 */
export class CouponUsageRecord extends Entity<CouponUsageRecordProps> {
  private _versionNumber: number = 1;

  public constructor(props: CouponUsageRecordProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CouponUsageRecordProps, id?: string): CouponUsageRecord {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CouponUsageRecord name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CouponUsageRecord code cannot be empty.");
    }

    return new CouponUsageRecord(
      {
        ...props,
  ...
  public static reconstitute(props: CouponUsageRecordProps, id: string, createdAt: Date, updatedAt: Date): CouponUsageRecord {
    return new CouponUsageRecord(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CouponUsageRecordProps>): void {
