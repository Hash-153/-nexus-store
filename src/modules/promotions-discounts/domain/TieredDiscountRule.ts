...
export interface TieredDiscountRuleProps {
...
/**
 * Domain Entity: TieredDiscountRule
 * Encapsulates state and business invariants for TieredDiscountRule within promotions-discounts.
 */
export class TieredDiscountRule extends Entity<TieredDiscountRuleProps> {
  private _versionNumber: number = 1;

  public constructor(props: TieredDiscountRuleProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: TieredDiscountRuleProps, id?: string): TieredDiscountRule {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("TieredDiscountRule name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("TieredDiscountRule code cannot be empty.");
    }
  ...
  public static reconstitute(props: TieredDiscountRuleProps, id: string, createdAt: Date, updatedAt: Date): TieredDiscountRule {
    return new TieredDiscountRule(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<TieredDiscountRuleProps>): void {
