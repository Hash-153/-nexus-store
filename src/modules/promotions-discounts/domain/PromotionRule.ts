...
/**
 * Domain Entity: PromotionRule
 * Encapsulates state and business invariants for PromotionRule within promotions-discounts.
 */
export class PromotionRule extends Entity<PromotionRuleProps> {
  private _versionNumber: number = 1;

  public constructor(props: PromotionRuleProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PromotionRuleProps, id?: string): PromotionRule {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PromotionRule name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PromotionRule code cannot be empty.");
    }
  ...
  public static reconstitute(props: PromotionRuleProps, id: string, createdAt: Date, updatedAt: Date): PromotionRule {
    return new PromotionRule(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PromotionRuleProps>): void {
