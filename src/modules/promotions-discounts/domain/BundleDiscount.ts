...
export interface BundleDiscountProps {
...
/**
 * Domain Entity: BundleDiscount
 * Encapsulates state and business invariants for BundleDiscount within promotions-discounts.
 */
export class BundleDiscount extends Entity<BundleDiscountProps> {
  private _versionNumber: number = 1;

  public constructor(props: BundleDiscountProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: BundleDiscountProps, id?: string): BundleDiscount {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("BundleDiscount name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("BundleDiscount code cannot be empty.");
    }
  ...
  public static reconstitute(props: BundleDiscountProps, id: string, createdAt: Date, updatedAt: Date): BundleDiscount {
    return new BundleDiscount(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<BundleDiscountProps>): void {
