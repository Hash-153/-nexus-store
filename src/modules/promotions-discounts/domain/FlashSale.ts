...
/**
 * Domain Entity: FlashSale
 * Encapsulates state and business invariants for FlashSale within promotions-discounts.
 */
export class FlashSale extends Entity<FlashSaleProps> {
  private _versionNumber: number = 1;

  public constructor(props: FlashSaleProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: FlashSaleProps, id?: string): FlashSale {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("FlashSale name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("FlashSale code cannot be empty.");
    }
  ...
  public static reconstitute(props: FlashSaleProps, id: string, createdAt: Date, updatedAt: Date): FlashSale {
    return new FlashSale(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<FlashSaleProps>): void {
