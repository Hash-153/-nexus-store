...
export interface CustomerGroupDiscountProps {
...
/**
 * Domain Entity: CustomerGroupDiscount
 * Encapsulates state and business invariants for CustomerGroupDiscount within promotions-discounts.
 */
export class CustomerGroupDiscount extends Entity<CustomerGroupDiscountProps> {
  private _versionNumber: number = 1;

  public constructor(props: CustomerGroupDiscountProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CustomerGroupDiscountProps, id?: string): CustomerGroupDiscount {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("CustomerGroupDiscount name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("CustomerGroupDiscount code cannot be empty.");
    }
  ...
  public static reconstitute(props: CustomerGroupDiscountProps, id: string, createdAt: Date, updatedAt: Date): CustomerGroupDiscount {
    return new CustomerGroupDiscount(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CustomerGroupDiscountProps>): void {
