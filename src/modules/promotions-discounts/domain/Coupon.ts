...
/**
 * Domain Aggregate Root: Coupon
 * Encapsulates state and business invariants for Coupon within promotions-discounts.
 */
export class Coupon extends AggregateRoot<CouponProps> {
  private _versionNumber: number = 1;

  public constructor(props: CouponProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: CouponProps, id?: string): Coupon {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("Coupon name cannot be empty.");
    }
  ...
  public static reconstitute(props: CouponProps, id: string, createdAt: Date, updatedAt: Date): Coupon {
    return new Coupon(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<CouponProps>): void {
