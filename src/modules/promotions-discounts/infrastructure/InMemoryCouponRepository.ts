...
import type { ICouponRepository, ICouponFilterOptions } from "../domain/ICouponRepository.ts";

/**
 * InMemory Repository for Coupon
...
export class InMemoryCouponRepository extends InMemoryRepository<Coupon> implements ICouponRepository {
  public async findByName(name: string): Promise<Coupon | null> {
  ...
  public async findFiltered(options: ICouponFilterOptions = {}): Promise<Coupon[]> {
  ...
  public async count(options: ICouponFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: Coupon[]): Promise<void> {
