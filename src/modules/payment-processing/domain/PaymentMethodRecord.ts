...
export interface PaymentMethodRecordProps {
...
/**
 * Domain Entity: PaymentMethodRecord
 * Encapsulates state and business invariants for PaymentMethodRecord within payment-processing.
 */
export class PaymentMethodRecord extends Entity<PaymentMethodRecordProps> {
  private _versionNumber: number = 1;

  public constructor(props: PaymentMethodRecordProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PaymentMethodRecordProps, id?: string): PaymentMethodRecord {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PaymentMethodRecord name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PaymentMethodRecord code cannot be empty.");
    }
  ...
  public static reconstitute(props: PaymentMethodRecordProps, id: string, createdAt: Date, updatedAt: Date): PaymentMethodRecord {
    return new PaymentMethodRecord(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PaymentMethodRecordProps>): void {
