...
export interface FraudAssessmentProps {
...
/**
 * Domain Entity: FraudAssessment
 * Encapsulates state and business invariants for FraudAssessment within payment-processing.
 */
export class FraudAssessment extends Entity<FraudAssessmentProps> {
  private _versionNumber: number = 1;

  public constructor(props: FraudAssessmentProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: FraudAssessmentProps, id?: string): FraudAssessment {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("FraudAssessment name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("FraudAssessment code cannot be empty.");
    }
  ...
  public static reconstitute(props: FraudAssessmentProps, id: string, createdAt: Date, updatedAt: Date): FraudAssessment {
    return new FraudAssessment(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<FraudAssessmentProps>): void {
