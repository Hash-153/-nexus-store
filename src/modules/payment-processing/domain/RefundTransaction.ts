...
export interface RefundTransactionProps {
...
/**
 * Domain Entity: RefundTransaction
 * Encapsulates state and business invariants for RefundTransaction within payment-processing.
 */
export class RefundTransaction extends Entity<RefundTransactionProps> {
  private _versionNumber: number = 1;

  public constructor(props: RefundTransactionProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: RefundTransactionProps, id?: string): RefundTransaction {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("RefundTransaction name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("RefundTransaction code cannot be empty.");
    }
  ...
  public static reconstitute(props: RefundTransactionProps, id: string, createdAt: Date, updatedAt: Date): RefundTransaction {
    return new RefundTransaction(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<RefundTransactionProps>): void {
