...
/**
 * Domain Aggregate Root: PaymentTransaction
 * Encapsulates state and business invariants for PaymentTransaction within payment-processing.
 */
export class PaymentTransaction extends AggregateRoot<PaymentTransactionProps> {
  private _versionNumber: number = 1;

  public constructor(props: PaymentTransactionProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PaymentTransactionProps, id?: string): PaymentTransaction {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PaymentTransaction name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PaymentTransaction code cannot be empty.");
    }
  ...
  public static reconstitute(props: PaymentTransactionProps, id: string, createdAt: Date, updatedAt: Date): PaymentTransaction {
    return new PaymentTransaction(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PaymentTransactionProps>): void {
