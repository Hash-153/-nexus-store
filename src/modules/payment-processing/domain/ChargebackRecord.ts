...
export interface ChargebackRecordProps {
...
/**
 * Domain Entity: ChargebackRecord
 * Encapsulates state and business invariants for ChargebackRecord within payment-processing.
 */
export class ChargebackRecord extends Entity<ChargebackRecordProps> {
  private _versionNumber: number = 1;

  public constructor(props: ChargebackRecordProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: ChargebackRecordProps, id?: string): ChargebackRecord {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("ChargebackRecord name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("ChargebackRecord code cannot be empty.");
    }
  ...
  public static reconstitute(props: ChargebackRecordProps, id: string, createdAt: Date, updatedAt: Date): ChargebackRecord {
    return new ChargebackRecord(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<ChargebackRecordProps>): void {
