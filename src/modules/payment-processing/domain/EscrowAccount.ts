...
/**
 * Domain Entity: EscrowAccount
 * Encapsulates state and business invariants for EscrowAccount within payment-processing.
 */
export class EscrowAccount extends Entity<EscrowAccountProps> {
  private _versionNumber: number = 1;

  public constructor(props: EscrowAccountProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: EscrowAccountProps, id?: string): EscrowAccount {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("EscrowAccount name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("EscrowAccount code cannot be empty.");
    }
  ...
  public static reconstitute(props: EscrowAccountProps, id: string, createdAt: Date, updatedAt: Date): EscrowAccount {
    return new EscrowAccount(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<EscrowAccountProps>): void {
