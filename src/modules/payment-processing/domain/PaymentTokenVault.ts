...
export interface PaymentTokenVaultProps {
...
/**
 * Domain Entity: PaymentTokenVault
 * Encapsulates state and business invariants for PaymentTokenVault within payment-processing.
 */
export class PaymentTokenVault extends Entity<PaymentTokenVaultProps> {
  private _versionNumber: number = 1;

  public constructor(props: PaymentTokenVaultProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PaymentTokenVaultProps, id?: string): PaymentTokenVault {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PaymentTokenVault name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PaymentTokenVault code cannot be empty.");
    }

    return new PaymentTokenVault(
      {
        ...props,
  ...
  public static reconstitute(props: PaymentTokenVaultProps, id: string, createdAt: Date, updatedAt: Date): PaymentTokenVault {
    return new PaymentTokenVault(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PaymentTokenVaultProps>): void {
