...
export interface GatewayWebhookLogProps {
...
/**
 * Domain Entity: GatewayWebhookLog
 * Encapsulates state and business invariants for GatewayWebhookLog within payment-processing.
 */
export class GatewayWebhookLog extends Entity<GatewayWebhookLogProps> {
  private _versionNumber: number = 1;

  public constructor(props: GatewayWebhookLogProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: GatewayWebhookLogProps, id?: string): GatewayWebhookLog {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("GatewayWebhookLog name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("GatewayWebhookLog code cannot be empty.");
    }

    return new GatewayWebhookLog(
      {
        ...props,
  ...
  public static reconstitute(props: GatewayWebhookLogProps, id: string, createdAt: Date, updatedAt: Date): GatewayWebhookLog {
    return new GatewayWebhookLog(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<GatewayWebhookLogProps>): void {
