...
/**
 * Domain Entity: EmailSubscriber
 * Encapsulates state and business invariants for EmailSubscriber within marketing-automation.
 */
export class EmailSubscriber extends Entity<EmailSubscriberProps> {
  private _versionNumber: number = 1;

  public constructor(props: EmailSubscriberProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: EmailSubscriberProps, id?: string): EmailSubscriber {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("EmailSubscriber name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("EmailSubscriber code cannot be empty.");
    }
  ...
  public static reconstitute(props: EmailSubscriberProps, id: string, createdAt: Date, updatedAt: Date): EmailSubscriber {
    return new EmailSubscriber(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<EmailSubscriberProps>): void {
