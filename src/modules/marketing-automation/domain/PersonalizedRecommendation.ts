...
export interface PersonalizedRecommendationProps {
...
/**
 * Domain Entity: PersonalizedRecommendation
 * Encapsulates state and business invariants for PersonalizedRecommendation within marketing-automation.
 */
export class PersonalizedRecommendation extends Entity<PersonalizedRecommendationProps> {
  private _versionNumber: number = 1;

  public constructor(props: PersonalizedRecommendationProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PersonalizedRecommendationProps, id?: string): PersonalizedRecommendation {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PersonalizedRecommendation name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PersonalizedRecommendation code cannot be empty.");
    }
  ...
  public static reconstitute(props: PersonalizedRecommendationProps, id: string, createdAt: Date, updatedAt: Date): PersonalizedRecommendation {
    return new PersonalizedRecommendation(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PersonalizedRecommendationProps>): void {
