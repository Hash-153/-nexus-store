...
export interface PriceDropSubscriptionProps {
...
/**
 * Domain Entity: PriceDropSubscription
 * Encapsulates state and business invariants for PriceDropSubscription within marketing-automation.
 */
export class PriceDropSubscription extends Entity<PriceDropSubscriptionProps> {
  private _versionNumber: number = 1;

  public constructor(props: PriceDropSubscriptionProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: PriceDropSubscriptionProps, id?: string): PriceDropSubscription {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("PriceDropSubscription name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("PriceDropSubscription code cannot be empty.");
    }

    return new PriceDropSubscription(
      {
        ...props,
  ...
  public static reconstitute(props: PriceDropSubscriptionProps, id: string, createdAt: Date, updatedAt: Date): PriceDropSubscription {
    return new PriceDropSubscription(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<PriceDropSubscriptionProps>): void {
