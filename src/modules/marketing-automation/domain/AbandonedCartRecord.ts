...
export interface AbandonedCartRecordProps {
...
/**
 * Domain Entity: AbandonedCartRecord
 * Encapsulates state and business invariants for AbandonedCartRecord within marketing-automation.
 */
export class AbandonedCartRecord extends Entity<AbandonedCartRecordProps> {
  private _versionNumber: number = 1;

  public constructor(props: AbandonedCartRecordProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: AbandonedCartRecordProps, id?: string): AbandonedCartRecord {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("AbandonedCartRecord name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("AbandonedCartRecord code cannot be empty.");
    }
  ...
  public static reconstitute(props: AbandonedCartRecordProps, id: string, createdAt: Date, updatedAt: Date): AbandonedCartRecord {
    return new AbandonedCartRecord(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<AbandonedCartRecordProps>): void {
