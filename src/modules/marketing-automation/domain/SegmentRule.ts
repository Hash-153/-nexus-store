...
export interface SegmentRuleProps {
...
/**
 * Domain Entity: SegmentRule
 * Encapsulates state and business invariants for SegmentRule within marketing-automation.
 */
export class SegmentRule extends Entity<SegmentRuleProps> {
  private _versionNumber: number = 1;

  public constructor(props: SegmentRuleProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: SegmentRuleProps, id?: string): SegmentRule {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("SegmentRule name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("SegmentRule code cannot be empty.");
    }
  ...
  public static reconstitute(props: SegmentRuleProps, id: string, createdAt: Date, updatedAt: Date): SegmentRule {
    return new SegmentRule(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<SegmentRuleProps>): void {
