...
export interface BackInStockSubscriptionProps {
...
/**
 * Domain Entity: BackInStockSubscription
 * Encapsulates state and business invariants for BackInStockSubscription within marketing-automation.
 */
export class BackInStockSubscription extends Entity<BackInStockSubscriptionProps> {
  private _versionNumber: number = 1;

  public constructor(props: BackInStockSubscriptionProps, id?: string, createdAt?: Date, updatedAt?: Date) {
  ...
  public static create(props: BackInStockSubscriptionProps, id?: string): BackInStockSubscription {
    if (props.name !== undefined && props.name.trim().length === 0) {
      throw new ValidationError("BackInStockSubscription name cannot be empty.");
    }
    if (props.code !== undefined && props.code.trim().length === 0) {
      throw new ValidationError("BackInStockSubscription code cannot be empty.");
    }
  ...
  public static reconstitute(props: BackInStockSubscriptionProps, id: string, createdAt: Date, updatedAt: Date): BackInStockSubscription {
    return new BackInStockSubscription(props, id, createdAt, updatedAt);
  }

  public updateDetails(updates: Partial<BackInStockSubscriptionProps>): void {
