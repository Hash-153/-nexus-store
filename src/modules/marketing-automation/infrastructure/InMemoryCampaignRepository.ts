...
import type { ICampaignRepository, ICampaignFilterOptions } from "../domain/ICampaignRepository.ts";

/**
 * InMemory Repository for Campaign
 * Thread-safe simulated repository with comprehensive query filtering.
 */
export class InMemoryCampaignRepository extends InMemoryRepository<Campaign> implements ICampaignRepository {
  ...
  public async findByCode(code: string): Promise<Campaign | null> {
  ...
  public async findFiltered(options: ICampaignFilterOptions = {}): Promise<Campaign[]> {
  ...
  public async count(options: ICampaignFilterOptions = {}): Promise<number> {
  ...
  public async saveBatch(entities: Campaign[]): Promise<void> {
